...
export function sellerConfigFromEnv(name: string): SellerConfig {
  return {
    name,
    services: (process.env.SERVICES ?? 'proofsentry').split(',').map((s) => s.trim()).filter(Boolean),
    floorSol: Number(process.env.FLOOR_SOL ?? '0.0003'),
    persona: process.env.PERSONA ?? 'an API reliability specialist selling reproducible endpoint evidence',
  }
}
